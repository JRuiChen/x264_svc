...
void x264_sps_init( x264_sps_t *sps, int i_id, x264_param_t *param )
{
    ...
    sps->i_id = i_id;
	/*sky 2014.08.29 Ìí¼Ó ssps µÄ¿í¶È¸ß¶È¼ÆËã*/
...
/*	sky 2014.08.28 Ìí¼Óssps Çé¿öÏÂ i_profile_idc*/
   ...
   /*sky 2014.08.28 sps init Ìí¼Ó ||sps->i_profile_idc == PROFILE_SCALABLE_HIGH_PROFILE;*/
    sps->b_constraint_set1  = (sps->i_profile_idc <= PROFILE_MAIN ||sps->i_profile_idc == PROFILE_SCALABLE_HIGH_PROFILE);

	/* Never set constraint_set2, it is not necessary and not used in real world. */
    sps->b_constraint_set2  = 0;
  
  /*sky 2014.08.21ÎªÁËºÍÊä³öÂëÁ÷¶ÔÓ¦´ÓÉÏÊöÓï¾ä¸ÄÎªÏÂÊö*/
  ...
  /*sky 2014.08.21£¬Ã»ÓÐ¿´¶®ååi_level_idcµÄÈ¡Öµ¹ØÏµºÍÒâÒå£¬ÎªÁË¸úÂëÁ÷¶ÔÉÏÌí¼ÓÈçÏÂ¾ä×Ó*/
	if(sps->i_nal_type == NAL_UNIT_SUBSET_SPS && sps->i_profile_idc == PROFILE_SCALABLE_HIGH_PROFILE)
		{
			sps ->i_level_idc =  21 ; // Õâ¸ö ÖµÊÇ¸ù¾Ýjsvm±àµÄÂëÁ÷´Õ³öÀ´µÄ
		}
 ...
 /*ÂëÁ÷¼ì²â¹ý*/
		sps ->b_inter_layer_deblocking_present = 0 ;//,ÂëÁ÷³õÖµ0 
		sps ->b_slice_header_restriction_flag = 0; // jsvm
		sps ->b_svc_vui_parameters_present_flag = 0; // Ð´ËÀ0
...
/*sky 2014.08.28 Ìí¼ÓÁË ||sps->i_profile_idc == PROFILE_SCALABLE_HIGH_PROFILE
		...
		||sps->i_profile_idc == PROFILE_SCALABLE_BASELINE_PROFILE)
    {
        ...
        bs_write1( s, 0 ); // seq_scaling_matrix_present_flagÕâ¸öµØ·½¸øËûÄ¬ÈÏ¸³ 0 ÁË
    }
	  ...
	  if(sps ->i_nal_type == NAL_UNIT_SUBSET_SPS)
  	{
			if(sps ->i_profile_idc == PROFILE_SCALABLE_BASELINE_PROFILE || sps ->i_profile_idc == PROFILE_SCALABLE_HIGH_PROFILE)
			{
				...
				bs_write1( s, sps ->b_slice_header_restriction_flag ) ;
					...
					/*------------svc yuvÀ©Õ¹ Ò»°ã±êÖ¾Î»Îª0²»½øÐÐ¼ÆËã-------------*/
...
int x264_sei_scalability_write(x264_t *h,bs_t *s)
{
	...
	int sei_payload = 29 ; //¾­ÑéÖµ,sky ,ÒªÐÞ¸Ä
	...
	int b_frm_rate_info_present_flag = 0; // Õâ¸ö¿ÉÒÔÔÙ²é
	int b_frm_size_info_present_flag = 1; 
	int b_layer_dependency_info_present_flag = 0;  // Õâ¸öflagÒªtrue
