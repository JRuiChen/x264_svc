                                    ...
                                    int i_idr_pic_id, int i_frame, int i_qp )
{
    ...
    sh->i_ref_layer_dq_id = -1; // 这个值初值为-1 ，具体还要赋值
...
//static void x264_slice_header_write( bs_t *s, x264_slice_header_t *sh, int i_nal_ref_idc) 
{
	...
	/*sky 2014.08.28 add sh writer nal扩展头*/
		...
		if(!sh->sps->b_slice_header_restriction_flag)
			{
				...
				bs_write1(s, 0); // store_ref_base_pic_flag prifix和这都写死了
				if((nal ->b_use_ref_base_pic_flag ||0)  // 这个||0是指store_ref_base_pic_flag
					&& !nal->b_idr_flag)
					{
						// 不会走这条线
					}
  ...
  if(!nal->b_no_inter_layer_pred_flag && nal->i_quality_id == 0)
  	{
		...
		bs_write1(s,sh->b_constrained_intra_resampling_flag); // 这个值不能确定，涉及到mb编码
...
/*sky 2014.08.28 sps pps init extension*/
	...
	*/
    ...
    if( x264_cqm_init( h ) < 0 )
        goto fail;
/*sky 2014.8.29 sps 对mb  w h  init*/
...
/*sky  2014.08.29 这个地方我注意到了，我觉得可以不用改*/
...
/* sky  2014.08.29 这个地方我注意到了，不知道需不要加上循环控制*/
...
static inline int x264_reference_update( x264_t *h )
{
	...
	/*sky 2014.08.29 我注意到了这个地方，但是我觉得可以不改。 基本层和扩展层这个值应该是一样的*/
...
static int x264_slice_write( x264_t *h )
{
	...
	/*sky 2014.08.29 修改sps[h->i_layer_id]*/
...
#define BS_BAK_ROW_VBV        2
    ...
    /* Slice */
/*sky 2014.08.28添加扩展层*/
/*这个地方不应该在这写，但是之前的i_type决定时应该不在循环内?
	而且需要判断他是不是条带信息*/
    /* Slice */
if(h->i_layer_id)
{
                             ...
                             x264_picture_t *pic_out )
{
    ...
    *pp_nal = NULL;
	...
	/*sky 2014.08.28 pps[]*/
		...
		/*sky 2014.08.29 我注意到了this sps，但是我觉得没有必要修改*/
